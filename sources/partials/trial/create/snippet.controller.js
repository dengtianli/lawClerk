(function () {
  angular
    .module("app.trial")
    .controller("TrialCreateController", TrialCreateController);

  TrialCreateController.$inject = [
    "$sce",
    "$scope",
    "$window",
    "$stateParams",
  ...
  function TrialCreateController(
    $sce,
    $scope,
    $window,
    $stateParams,
    ...
    vm.http = {
      create: function () {
        $scope.$on("wiserv-trial-response-article", function (event, data) {
          ...
          var articleStatus = JSON.stringify(data);
          //date
          var now = function() {
            var now = new Date();
          ...
          //获取到庭人员
          $scope.$parent.Trial.event.verify.verify_participant.getToCourt();
          //获取未到庭人员
          $scope.$parent.Trial.event.verify.verify_participant.getAbsentee();
          //将临时用于取文本的span标签隐藏
          $(".temporary").css({
          ...
          var articleTemplate = angular.element("article.trial").html();
            //将临时用于取文本的span标签隐藏
            $(".temporary").css({
              display: "inline-block"
            });
          trialCreateService.create({
              record_title: "",
              record_type: "section",
              ...
              record_js_status: articleStatus || "{}",
            })
            .then(function (result) {
              layer.msg(result.head.message);
            })
            .then(function () {
              $scope.$emit("wiserv-trial-get-list");
            })
            .then(function () {
              $state.go("trial");
            })
        });
        $scope.$emit("wiserv-trial-request-article", "parent");
      },
    };
